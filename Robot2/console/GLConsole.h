...
public:
    GLConsole(SDL_Window& window);
    ~GLConsole();

    // Use this before using commands.
    ...
    void ScrollUpPage();
    void ScrollDownPage();
    void SpecialFunc(int key, uint16_t modifiers);

    void PrintAllCVars();

    /// Add a character to the command line.
    void KeyboardFunc(unsigned char key, uint16_t modifiers);
...
inline void GLConsole::SetEnvironment(Environment* env)
{
    this->commands.SetEnvironment(env);
}
...
inline void GLConsole::Init()
{
    ...
    SDL_GetWindowSize(&this->window, &m_Viewport.width, &m_Viewport.height);
